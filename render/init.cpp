...
void vkinit(bool print_vkfeatures)
{
...

  {
    ...
    initial_win_width, initial_win_height, "vulkan", nullptr, nullptr);
    if (!window)
    {
...

  {
    ...
    for (u32 i = 0; i < num_queue; ++i)
    {
      ...
      {
        if (queue_fam_index_list[i] == unique_queue_fam_indices[k])
        {
          newval = false;
          break;
        }
    ...
    for (u32 qf = 0; qf < num_unique_queue_fam_indices; ++qf)
    {
      ...
      queue_create_infos[qf].queueFamilyIndex = unique_queue_fam_indices[qf];
